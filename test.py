...
class River:
    ...
    def Historical(self, SYMBOL):
        ...
        df = pd.DataFrame \
            (mt5.copy_rates_range(SYMBOL, self.TIMEFRAME_5M, lookback + self.sec_to_shift, looknow + self.sec_to_shift))
    ...
    def Bull_Eng(self, SYMBOL):
        ...
        Bullish_Engulfing = df['close'].iloc[-2] > df['open'].iloc[-3] and df['open'].iloc[-2] <= df['close'].iloc
            [-3] and (0.8 * 0.9) > 0.8 and \
                            df['close'].iloc[-3] < df['open'].iloc[-3] and df['close'].iloc[-2] > df['open'].iloc
                                [-2] and df['high'].iloc[-2] > df['high'].iloc[-3]
    ...
    def Bear_Eng(self, SYMBOL):
        self.SYMBOL = SYMBOL
        df = self.Historical(SYMBOL)
        Bearish_Engulfing = df['close'].iloc[-2] < df['open'].iloc[-3] and df['open'].iloc[-2] >= df['close'].iloc
            [-3] and (0.8 * 0.9) < 0.8 and \
                            df['close'].iloc[-3] > df['open'].iloc[-3] and df['close'].iloc[-2] < df['open'].iloc
                                [-2] and df['low'].iloc[-2] < df['low'].iloc[-3]
